...
def SurveyInterface(request):
    try:
        ...
        spl = cmd.fetchall()
        print(spl)
        return render(request, "surveyinterface.html", {'doctorname':doctor[0]['doctorname'],'icon':doctor[0]['icon'], 'specializationid': doctor[0]['specialization'],'splname':spl[0]['specialization']})
    except Exception as e:
        print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
        print(e)
